...
fn validate_rebalance_route(
    ...
    match (source_protocol.to_uppercase().as_str(), target_protocol.to_uppercase().as_str(), chain_id) {
        ("AAVE", _, ARBITRUM_CHAIN_ID) => {
            return Err("AAVE is not supported on Arbitrum. AAVE is only available on Sepolia.".to_string())
        },
        (_, "AAVE", ARBITRUM_CHAIN_ID) => {
            return Err("AAVE is not supported on Arbitrum. AAVE is only available on Sepolia.".to_string())
        },
        ("COMPOUND", _, SEPOLIA_CHAIN_ID) => {
            return Err("Compound is not supported on Sepolia. Compound is only available on Arbitrum.".to_string())
        },
        (_, "COMPOUND", SEPOLIA_CHAIN_ID) => {
            return Err("Compound is not supported on Sepolia. Compound is only available on Arbitrum.".to_string())
        },
        _ => {}
    }
