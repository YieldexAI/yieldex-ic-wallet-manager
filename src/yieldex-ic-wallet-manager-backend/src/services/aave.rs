...
pub async fn supply_link_to_aave_with_permissions(
    ...
    let link_address = address!("779877A7B0D9E8603169DdbD7836e478b4624789");
...
pub async fn withdraw_link_from_aave_with_permissions(
    ...
    if alink_balance_wei < amount_wei {
        return Err(format!("Insufficient aLINK balance. Have: {} wei, Need: {} wei", alink_balance_wei, amount_wei));
    }
        ...
        Ok(builder) => {
            ...
            match tx_response {
                Some(tx) => {
                    ...
                    // Обновляем daily usage
                    if let Err(e) = set_daily_usage(permissions_id, AAVE_POOL_ADDRESS.to_string(), amount_wei.to::<u64>(), user_principal) {
                        ic_cdk::println!("Warning: Failed to update daily usage: {}", e);
                    }
                    
                    Ok(format!("Successfully withdrew {} LINK from AAVE. Transaction: {:?}", amount_human, tx_hash))
                }
                None => Err("Transaction not found after sending".to_string()),
            }
        }
        Err(e) => Err(format!("Withdraw transaction failed: {:?}", e)),
    }
}
